...
class KkGDvGsXdController extends Controller
{
            ...
            if(session('admin')->level == 'T' || session('admin')->level == 'H') {
                ...
                if($inputs['phanloai']=='cho_nhan') {
                    if(session('admin')->level == 'T'  && session('admin')->sadmin == 'ssa') {
                        $model = KkGDvGs::where('trangthai', 'Chờ nhận')
                ...
                elseif($inputs['phanloai'] == 'duyet') {
                    if(session('admin')->level == 'T'  && session('admin')->sadmin == 'ssa') {
                        $model = KkGDvGs::where('trangthai', 'Duyệt')
                            ->whereYear('ngaychuyen', $inputs['nam'])
                            ->get();
                    }else{
                        $model = KkGDvGs::where('trangthai', 'Duyệt')
                ...
                elseif($inputs['phanloai'] == 'cong_bo') {
                    ...
                    if(session('admin')->level == 'T'  && session('admin')->sadmin == 'ssa') {
                        $model = CbKkDvGs::whereYear('ngaychuyen', $inputs['nam'])
                            ->get();
                    }else{
                        $model = CbKkDvGs::where('cqcq',session('admin')->cqcq)
                            ->whereYear('ngaychuyen', $inputs['nam'])
                            ->get();
                    }
                }elseif($inputs['phanloai']='bi_tra_lai'){
                    if(session('admin')->level == 'T'  && session('admin')->sadmin == 'ssa') {
                        $model = KkGDvGs::where('trangthai', 'Bị trả lại')
                            ->whereYear('ngaychuyen', $inputs['nam'])
                            ->get();
                    }else{
                        $model = KkGDvGs::where('trangthai', 'Bị trả lại')
                ...
                return view('manage.dvgs.kkgia.xetduyet.index')
                    ->with('model', $model)
    ...
    public function tralai(Request $request){
        if (Session::has('admin')) {
            ...
            if (session('admin')->level == 'T' || session('admin')->sadmin == 'ssa' || $model->cqcq == session('admin')->cqcq) {
                ...
                return redirect('xet_duyet_ke_khai_gia_sua');
            }else {
                return view('errors.perm');
            }
        } else
    ...
    public function getTtKkGs(Request $request){
        $result = array(
            'status' => 'fail',
            'message' => 'error',
        );
        ...
        if(isset($inputs['id'])){
            ...
            $modelhs = KkGDvGs::where('id',$inputs['id'])
                ->first();
            $modeldn = DnDvGs::where('masothue',$modelhs->masothue)->first();
    ...
    public function congbo($id){
        if (Session::has('admin')) {
            $modelkk = KkGDvGs::findOrFail($id);
            $modeldel = CbKkDvGs::where('masothue',$modelkk->masothue)
                ->delete();
            $model = new CbKkDvGs();
            ...
            $model->nguyennhandc = $modelkk->nguyennhandc;
