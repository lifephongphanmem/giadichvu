...
class KkGDvGsController extends Controller
{
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'T' || session('admin')->level == 'H') {
                if (session('admin')->sadmin == 'ssa') {
                    ...
                    $model = DnDvGs::where('cqcq', session('admin')->cqcq)
                        ->get();
                }
                return view('manage.dvgs.kkgia.ttdn.index')
                    ->with('model', $model)
        ...
        if (Session::has('admin')) {
            ...
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || $inputs['masothue'] == session('admin')->mahuyen) {
                ...
                if ($model->create($inputs)) {
                    $modelhh = KkGDvGsCtDf::where('masothue', $inputs['masothue'])
                        ->get();
                ...
                return redirect('ke_khai_gia_sua?&masothue=' . $inputs['masothue']);
            } else {
                return view('errors.perm');
            }
        } else
    ...
    public function edit($id)
    {
        if (Session::has('admin')) {
            ...
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || $model->masothue == session('admin')->mahuyen) {
                $modelct = KkGDvGsCt::where('mahs', $model->mahs)
                    ->get();
                return view('manage.dvgs.kkgia.kkgiadv.edit')
                    ->with('model', $model)
                    ->with('modelct', $modelct)
                    ->with('pageTitle', 'Chỉnh sửa thông tin kê khai mặt hàng sữa');
            } else {
                return view('errors.perm');
            }
        } else
    ...
    public function delete(Request $request)
    {
        if (Session::has('admin')) {
            ...
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || $model->masothue == session('admin')->mahuyen) {
                if ($model->delete()) {
                    $modelct = KkGDvGsCt::where('mahs', $model->mahs)->delete();
                }
                return redirect('ke_khai_gia_sua?&masothue=' . $model->masothue);
            } else {
                return view('errors.perm');
            }
        } else
    ...
    public function checkngay(Request $request)
    {
        ...
        if (isset($inputs['id'])) {
            $model = KkGDvGs::where('id', $inputs['id'])
                ->first();
