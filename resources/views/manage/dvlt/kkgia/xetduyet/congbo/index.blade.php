        ...
        <div class="col-md-2">
            <div class="form-group">
                ...
                {{-- <select name="namhs" id="namhs" class="form-control">
                    @if ($nam_start = intval(date('Y')) - 10)
                    @endif
                    @if ($nam_stop = intval(date('Y')) + 1)
                    @endif
                    @for ($i = $nam_start; $i <= $nam_stop; $i++)
                        <option value="{{ $i }}" {{ $i == $nam ? 'selected' : '' }}>Năm {{ $i }}
                        </option>
                    @endfor
                </select> --}}
            </div>
                            ...
                            <tbody>
                                @foreach ($model as $key => $tt)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($tt->phanloai == 'DT')
                                                <a href="{{ url('ke_khai_dich_vu_luu_tru/report_ke_khai/khach_san/' . $tt->mahs) }}"
                                                    target="_blank" class="btn btn-default btn-xs mbs"><i
                                                        class="fa fa-eye"></i>&nbsp;Xem chi tiết</a>
                                            @else
                                                <a href="{{ url('ke_khai_dich_vu_luu_tru/report_ke_khai/' . $tt->mahs) }}"
                                                    target="_blank" class="btn btn-default btn-xs mbs"><i
                                                        class="fa fa-eye"></i>&nbsp;Xem chi tiết</a>
                                            @endif
                                            @if (can('kkdvlt', 'approve'))
                                                <button type="button" onclick="confirmNhanHsedit({{ $tt->id }})"
                                                    class="btn btn-default btn-xs mbs" data-target="#nhanhsedit-modal"
                                                    data-toggle="modal"><i class="fa fa-edit"></i>&nbsp;
                                                    Chỉnh sửa TT nhận</button>
                                                <button type="button" onclick="getTtHuyDuyet({{ $tt->id }})"
                                                    class="btn btn-default btn-xs mbs" data-target="#huyduyet-modal"
                                                    data-toggle="modal"><i class="fa fa-stop"></i>&nbsp;
                                                    Huỷ duyệt HS</button>
                                                <a href="{{ url('ke_khai_dich_vu_luu_tru/' . $tt->macskd . '/hsdakk') }}"
                                                    target="_blank" class="btn btn-default btn-xs mbs"><i
                                                        class="fa fa-eye"></i>&nbsp;Thông tin hồ sơ đã kê khai</a>
                                            @endif
                                            <a href="{{ url('ke_khai_dich_vu_luu_tru/' . $tt->mahs . '/history') }}"
                                                target="_blank" class="btn btn-default btn-xs mbs"><i
                                                    class="fa fa-eye"></i>&nbsp;Lịch sử</a>
                                            @if ($tt->giaycnhangcs != '')
                                                <a href="{{ url('images/cskddvlt/hangcslt/' . $tt->giaycnhangcs) }}"
                                                    target="_blank" class="btn btn-default btn-xs mbs"><i
                                                        class="fa fa-eye"></i>&nbsp;Giấy công nhận hạng cơ sở lưu trú</a>
                                            @else
